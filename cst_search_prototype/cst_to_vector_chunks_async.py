...
# Always use high-performance packages
import nltk
...
class CSTToVectorChunks:
    ...
    def convert_cst_to_chunks(self, cst_data: Dict[str, Any]) -> List[CodeChunk]:
        ...
        # Second pass: extract chunks with concurrent processing
        print(f"⚡ Extracting semantic chunks from {len(files_data)} files using 4 workers...")
        ...
        def process_file(file_data):
            ...
            return self._extract_file_chunks(filepath, language, cst_tree)
        
        # Process files concurrently
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_file = {
                executor.submit(process_file, file_data): i 
            ...
            for future in as_completed(future_to_file):
                ...
                try:
                    ...
                    if completed % 50 == 0:
                        print(f"   📈 Processed {completed}/{len(files_data)} files...")
                except Exception as e:
                    print(f"   ❌ Error processing file {file_idx}: {e}")
        
        print(f"✅ Completed processing {len(files_data)} files")
        
        # Check for and fix duplicate IDs
        all_chunks = self._fix_duplicate_ids(all_chunks)
        
        return all_chunks
    
    def _fix_duplicate_ids(self, chunks: List[CodeChunk]) -> List[CodeChunk]:
        """Fix any duplicate IDs in chunks"""
        seen_ids = set()
        fixed_chunks = []
        duplicate_count = 0
        ...
        for chunk in chunks:
            ...
            if original_id in seen_ids:
                ...
                unique_suffix = hashlib.md5(f"{chunk.filepath}:{chunk.text[:100]}".encode()).hexdigest()[:8]
                new_id = f"{original_id}_{unique_suffix}"
                
                # Ensure this new ID is also unique
                counter = 1
                while new_id in seen_ids:
                    new_id = f"{original_id}_{unique_suffix}_{counter}"
                    counter += 1
    ...
    def _create_semantic_chunk(self, filepath: str, language: str, node: Dict[str, Any],
        ...
        # Create unique ID using filepath, type, name, line, and byte position
        import hashlib
        id_components = f"{filepath}:{node_type}:{name}:{start_line}:{start_byte}"
        id_hash = hashlib.md5(id_components.encode()).hexdigest()[:8]
        unique_id = f"{filepath}:{node_type}:{name}:{start_line}_{id_hash}"
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python cst_to_vector.py <cst_output.json> [output_directory]")
        sys.exit(1)
    
    input_file = sys.argv[1]
    output_dir = sys.argv[2] if len(sys.argv) > 2 else "vector_output"
    ...
    print(f"\n🎯 Next steps:")
    print(f"   1. Run: python vector_search.py load {output_file}")
    print(f"   2. Run: python vector_search.py query 'your search query'")
